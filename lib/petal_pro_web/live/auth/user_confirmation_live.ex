defmodule PetalProWeb.UserConfirmationLive do
      ...
      :error ->
        ...
        case socket.assigns do
          %{current_user: %{confirmed_at: confirmed_at}} when not is_nil(confirmed_at) ->
            {:noreply,
             redirect(socket, to: PetalProWeb.Helpers.home_path(socket.assigns.current_user))}
  ...
  defp get_after_confirm_redirect_path(%{assigns: %{current_user: current_user}})
    ...
    invitations = PetalPro.Orgs.list_invitations_by_user(current_user)

    if Enum.any?(invitations),
      do: ~p"/app/users/org-invitations",
      else: PetalProWeb.Helpers.home_path(current_user)
  end

defmodule PetalProWeb.Layouts do
  ...
  def app_name, do: PetalPro.config(:app_name)
  ...
  def description(conn) do
    if conn.request_path == "/" do
      PetalPro.config(:seo_description)
    else
      if is_public_page(conn.request_path) do
        Logger.warning(
  ...
  def current_page_url(%{request_path: request_path}),
    do: PetalProWeb.Endpoint.url() <> request_path

  def current_page_url(_conn), do: PetalProWeb.Endpoint.url()
  ...
  def twitter_site(_conn) do
    if PetalPro.config(:twitter_url) do
      "@" <> (PetalPro.config(:twitter_url) |> String.split("/") |> List.last())
    else
      ""
    end
  end
  ...
  def is_public_page(request_path) do
    request_path != "/" &&
      PetalProWeb.Menus.public_menu_items()
      |> Enum.map(& &1.path)
